...
const Banco = require('../auxiliares/Banco');

const getsSearch = require('./getsSearch');

const getPages = async (req, res) => {

  const getPages = await Banco.getPages(req.params);
  res.status(200).send(getPages);
}

const get = async (req, res) => {

  const get = await Banco.get(req.params);
...
router.post('/:idAllow', async (req, res, next) => {
  ...
  const banco = new Banco(req.body, req.params);
  const post = await banco.post();
...
router.delete('/:idAllow/:_id', async (req, res, next) => {
  ...
  const banco = new Banco(req.params, req.params);
  const del = await banco.delete();
